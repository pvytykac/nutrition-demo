...
public class ModelNameMinLengthFilterStage implements Stage.FilterStage<HardwareModel> {

    private final int minLength;

    @Override
    public String getId() {
        return this.getClass().getSimpleName() + "#" + minLength;
    }

    @Override
    public boolean matches(HardwareModel input) {
        var result = input.model().length() >= this.minLength;

        Pipeline.getContextBuilder()
                .debugInfo(this, Map.of("input", input.model(), "minLength", minLength, "output", result));
